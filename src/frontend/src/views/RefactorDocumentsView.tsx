...
export function RefactorDocumentsView() {
  const { projectId } = useParams<{ projectId: string }>();
  const navigate = useNavigate();
  const { documents, loading, error, refreshDocumentsForProject } = useRefactorDocuments();
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <RefactorDocumentsDataTable onDocumentSelect={handleSelectDocument} />
        </CardContent>
      </Card>
    </div>
  );
